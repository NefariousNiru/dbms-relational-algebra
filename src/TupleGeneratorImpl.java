                ...
                for (var j = 0; j < tuples[i]; j++) {
                    for (var k = 0; k < attribute.length; k++) {
                        if (pKeys.contains (attribute[k])) {  // key requires uniqueness
                            ...
                            switch (domain[k]) {
                            case "Integer":
                                for (iVal = rand.nextInt (100_000_000); pKeyValues.contains (iVal);
                                     iVal = rand.nextInt (100_000_000));
                                subResult[j][k] = iVal;
                                pKeyValues.add (iVal);
                                break;
                            case "String":
                                for (sVal = attribute[k] + rand.nextInt (100_000_000); pKeyValues.contains(sVal);
                                     sVal = attribute[k] + rand.nextInt (100_000_000));
                                subResult[j][k] = sVal;
                                pKeyValues.add (sVal);
                                break;
                            case "Double":
                                for (dVal = rand.nextInt (100_000_000) * rand.nextDouble (); pKeyValues.contains( dVal);
                                     dVal = rand.nextInt (100_000_000) * rand.nextDouble ());
                                subResult[j][k] = dVal;
                                pKeyValues.add (dVal);
                                break;
                            default:
                            ...
                            switch (domain[k]) {
                            case "Integer":
                                subResult[j][k] = rand.nextInt(100_000_000); break;
                            case "String":
                                subResult[j][k] = attribute [k] + rand.nextInt (100_000_000); break;
                            case "Double":
                                subResult[j][k] = rand.nextInt (100_000_000) * rand.nextDouble (); break;
                            default:
                                throw new IllegalArgumentException("Invalid type in switch: " + domain[k]);
                            } // switch
