...
//        case BPTREE_MAP  -> new BpTreeMap <> (KeyType.class, Comparable [].class);
    ...
    public Table project (String attributes)
    {
        ...
        for (var tuple : tuples) {
            var newTuples = new Comparable[attrs.length]; //array hold projected tuple
            //find corresponding col for each attr and add it to new tuple
            for (int i = 0; i < attrs.length; i++) {
                newTuples[i] = tuple[col(attrs[i])];
            }
            rows.add(newTuples); //add new tuples into rows
        }
